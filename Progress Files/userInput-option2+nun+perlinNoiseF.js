...
// Perlin noise flow field for Confess
...
// Perlin noise layer for Confess

// ─────────────────────────────────────────────

function initPerlinParticles() {

  particles = [];

  flowField = [];

  flowCols = floor(width / gridSize);

  flowRows = floor(height / gridSize);
...
function drawPerlinNoiseLayer() {

  if (!perlinActive && perlinAlpha <= 0) return;
  ...
  perlinAlpha = constrain(perlinAlpha, 0, 255);
...
class FlowParticle {
  ...
  perlinLayer.stroke(r, g, b, alphaValue * 0.18);

  perlinLayer.strokeWeight(5.8);
